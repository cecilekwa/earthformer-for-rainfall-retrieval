...
* Other data that is used is the Digital Elevation Model (DEM), which you can be obtained from the NASA Earthsystem database and the longitude and latitude grid of the area investigated. 

* The benchmark IMERG-Early can be downloaded using the same script as the `preprocess_data/1.data_retrieval/imerg_retrieval.ipynb` script. 

### 2) Resampling the data
* If the SEVIRI data was not reprojected yet, the `preprocess_data/2.resampling/seviri_retrieval_reproject_nat_to_hdf5.ipynb` can be used to reproject the geostationary projection into WGS84 projection.
* The IMERG data can be resampled to the same resolution as the SEVIRI data using the `pre_process_data/2.resampling/imerg_resampling.ipynb` script. 

### 3) Selecting data
The selection of data is based on the values of the IMERG data. It is done in a two-step approach.
* First, all the selection criteria are saved into a dataframe, so later, the selection criteria can be decided upon more iteratively. This is done using the `preprocess_data/3.event_selection_file_merging/1.retrieve_selection_criteria.ipynb` script. 
* Secondly, the selected timestamps are used to merge the SEVIRI data and IMERG data into one file, which is currently required for using the model. This is done, using the `preprocess_data/3.event_selection_file_merging/2.event_selection_merging_files.ipynb`.
...
The script to split the data into a training, test and validation dataset and save it into the correct directories using the script `preprocess_data/4.data_splitting/train_test_val_splitting.ipynb`.

### 5) Normalizing data
The SEVIRI data is normalized for better performance of the model. The IMERG data is log transformed in the `utils/dataUtils.py` script of the model. To obtain the mean and standard deviation of the entire dataset of the seviri dataset the `preprocess_data/5.calc_normalize_values/norm_values_input_data.ipynb` can be used. 
...
## Running the model
In order to run the trained model for inference, download the pretrained weights provided in [https://doi.org/10.5281/zenodo.14993091](https://doi.org/10.5281/zenodo.14993091) and unzip the file `ef_inca_multisource2precip.pt` into `trained_ckpt`. 
